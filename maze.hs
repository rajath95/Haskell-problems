...
-- Generate a list of all states, i.e., (row, column) maze positions, that can be directly reached from the current state (position)
-- in the maze problem. Those are the positions one row above or below or two columns to the left or the right that do not contain a wall ('#')
check :: [[Char]] -> Int -> Int ->  [(Int,Int)]
check maze row col
            | row < 0 || col < 0 = []
            | row > length(maze) || col > length(maze!!0) = []
            | (maze !! (row) !! (col)) == ' ' = [(row,col)]
            | otherwise = []


genStatesMaze :: [[Char]] -> (Int, Int) -> [(Int, Int)]
genStatesMaze maze (row, col) = check maze (row+1) col ++ check maze row (col+2) ++ check maze (row-1) col ++ check maze row (col-2)
...
h'Maze curr goal =  (fst goal - fst curr)
